...
# Cargando el dataset
dataset = pd.read_csv('bill_authentication.csv')  # Lee el archivo CSV con los datos
X = dataset[['Variance','Skewness']].values   # Extrae las características (Edad, Salario Estimado)
y = dataset['Class'].values   # Extrae la variable objetivo (Compra o No Compra)
...
classifier = RandomForestClassifier(n_estimators = 10, criterion = 'entropy', random_state = 3500)  # Usamos 'entropy' como criterio
classifier.fit(X_train, y_train)  # Ajusta el modelo a los datos de entrenamiento

# Predicción de un nuevo resultado (Ejemplo: varianza = 5.3063, oblicuidad = 5.2684)
resultado = classifier.predict(sc.transform([[5.3063, 5.2684]]))  # Predice si comprará o no
print(f"Predicción para  varianza = 5.3063 y oblicuidad = 5.2684: {resultado}")
